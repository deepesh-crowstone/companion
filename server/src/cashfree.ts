...
export function buildIntimacyOrderId(userId: number, targetLevel: number): string {
  const suffix = uuidv4().replace(/-/g, "").slice(0, 12);
  return `intimacy_u${userId}_l${targetLevel}_${suffix}`;
}
...
export async function createCashfreeOrder(options: {
  ...
  const res = await fetch(`${cashfreeBaseUrl()}/orders`, {
    ...
    body: JSON.stringify({
      ...
      order_note: `Zara intimacy unlock`,
    }),
  });
